...
def write_report(stability: pd.DataFrame, event_ic: pd.DataFrame, sensitivity: pd.DataFrame, tail: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- {len(loo)} 组 leave-one-date-out：最小/最大 median IC={loo.estimate.min():.3f}/{loo.estimate.max():.3f}；是否全部正向：{'是' if loo_stable else '否'}。",
...
def main() -> int:
    ...
    selected = runner.select_batch36(HERE / "inputs/event_inventory.parquet")
