...
def select_batch36(inventory_path: Path) -> pd.DataFrame:
    """Pre-result deterministic sample: four activity quantiles per each date."""
    ...
    targets = ((0.05, "low"), (0.35, "medium"), (0.65, "medium"), (0.95, "high"))
    for event_date, date_rows in inventory.groupby("event_date", sort=True):
        date_rows = date_rows.reset_index(drop=True)
        available = set(date_rows.index)
        for quantile, activity in targets:
            ...
            row["sample_activity"] = activity
            selected.append(row)
            used_cities.add(str(row.city))
            available.remove(idx)
    result = pd.DataFrame(selected).reset_index(drop=True)
    if len(result) != 36 or result.event_date.nunique() != 9:
        raise RuntimeError(f"batch sample must be 36 events over 9 dates, got {len(result)} over {result.event_date.nunique()}")
...
def write_outputs(selected: pd.DataFrame, summaries: list[dict[str, Any]], started: float) -> None:
    ...
    failures = [row for summary in summaries for row in summary["failures"]]
    ...
    lines = [
        ...
        f"- 失败 token：{len(failures)}；明细见 `compact/batch_36_event/token_failures.csv`。主要原因是 PMXT tick_size_change 的 old tick 与回放状态冲突，未自动修补。",
    ...
    score = [
        ...
        f"- Stop condition：所有 crossing 为负；失败 token={len(failures)}；skipped token={len(skipped)} 且偏向早期 clean；概率和偏差 P95={sum_dev_p95:.3f}。",
    ]
