...
def write_reports(output: Path, selected: list[dict[str, Any]], results: list[dict[str, Any]], head: pd.DataFrame,
    ...
    elif parity and parity[0]["status"] == "not_run":
        parity_text = f"NOT RUN：{parity[0].get('reason', parity[0].get('error', 'unknown'))}；按预注册规则暂不扩张。"
    else:
        parity_text = f"FAIL：{parity[0].get('error', 'unknown')}；按预注册规则暂不扩张。"
    ...
    lines += ["", "obs200 虽显著降低 zero rate，但 elapsed P90 超过 1800s，因此不满足 amendment 候选门槛。", "", "## direct replay parity", "", parity_text, "具体记录见 `compact/pilot_6_event/direct_native_parity.json`。", "", "## E3", "", f"- 因子统计路径建议：**{decision}**。", f"- 扩张决策：**{'GO' if parity_pass and event_failures == 0 else 'NO-GO（先补 parity）'}**。", "- 6-event pilot 只验证流程和方向，不宣称发现可交易 alpha。", ""]
    (report_dir / "pilot_report.md").write_text("\n".join(lines), encoding="utf-8")

    score = ["# Boss scorecard：PMXT 天气因子 pilot", "", f"- 阶段：6-event pilot；城市 {len({str(e['city']) for e in selected})}；clean 3 / degraded 3。", f"- Runtime：{wall/60:.1f} 分钟；event failure：{event_failures}/6。", f"- 因子统计路径：**{decision}**；扩张：**{'GO' if parity_pass and event_failures == 0 else 'NO-GO'}**。", "- E2：价格分层样本不足，只能诊断；不得当作正式结论。", "- valid_obs：diagnostic-only，未触发 amendment。", f"- direct→Nautilus parity：{parity_text}", "", "## 主指标", "", "| factor | horizon | coverage | zero | median IC | positive event | activity IC diff | gate |", "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |"]
    for row in merged.itertuples():
